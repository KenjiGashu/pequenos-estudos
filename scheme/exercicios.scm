...
(define (equ? x y)
  ((get 'equ? 'generic) x y))
(define (make-polar-real x y)
  ((get 'make-from-real-imag 'complex) x y))
(make-polar-real 1 2)
(equ? (make-polar-real 1 2) (make-polar-real 1 2))
...
(define (coerce-list list)
  ...
  (if (null? list)
      '()
      (let ((type2 (type-tag (car list)))
	    (num (contents (car list))))
	(let ((conv (get-coercion type2 type)))
	  (cond (conv (cons (conv num) (coerce-to-type type (cdr list))))
		((eq? type2 type) (cons (car list) (coerce-to-type type (cdr list))))
		(else (cons #f '())))))))
  (define (iter cur list)
    (let ((coerced-list (coerce-to-type (type-tag (car cur)) list)))
      (if (problem? coerced-list)
	  (iter (cdr cur) list)
	  coerced-list)))
  (define (problem? list)
    "detecta se o coerce list deu erro"
    (cond ((null? list) #f)
	  ((not (car list)) #t)
...
(generic-raise-list lista-scheme-number)
;; exercise 2.84
(define (apply-generic op . args)
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags)))
...
(define (install-complex-package)
  ...
  (define (equ? x y)
    (apply-generic 'equ? x y))
  ;; adicionado acesso as partes reais, imaginarias, magnitude e angulo
  (define (real-part x)
