(ns caixa.middleware
  (:require [caixa.env :refer [defaults]]
            [cheshire.generate :as cheshire]
            [cognitect.transit :as transit]
            [clojure.tools.logging :as log]
            [caixa.layout :refer [error-page]]
...
(def date-writer
  (transit/write-handler
    (constantly "muuntaja")
    (fn [v] (-> ^java.util.Date v .getTime))
    (fn [v] (-> ^java.util.Date v .getTime .toString))))
...
(def restful-date-format-options
  (update
    muuntaja/default-options
    :formats
    merge
    {"application/json"
     ...
     {:decoder [(partial transit-format/make-transit-decoder :json)]
      :encoder [#(transit-format/make-transit-encoder
                   ...
                   (merge
                     %
                     {:handlers {java.util.Date date-writer}}))]}}))
